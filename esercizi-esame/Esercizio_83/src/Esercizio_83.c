...
Scrivere un programma C in cui il processo padre apre due pipe e crea un processo figlio. Il padre apre un file e legge
 riga per riga il contenuto inviando la riga letta al processo figlio. Questo elimina tutte le vocali dalla linea letta dalla
 pipe e rinvia la stringa risultante sulla pipe al padre. Il padre stampa la stringa ricevuta e legge la prossima linea del file.
 Quanto raggiunge l’EOF manda la stringa “%%%” ed aspetta la terminazione del figlio. Il processo figlio ricevuta
 la stringa chiude le pipe e termina restituendo al padre il numero di vocali eliminate
 */
